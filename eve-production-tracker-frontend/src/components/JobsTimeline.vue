...
/* --- 1. КЛЮЧЕВОЙ КОНТЕЙНЕР ПРОКРУТКИ --- */
...
.time-headers {
  ...
  /* 🔴 САМЫЕ ВАЖНЫЕ ИЗМЕНЕНИЯ С !important */
  height: 40px !important; /* Принудительная высота */
  min-height: 40px !important; /* Запрет на сжатие */
  max-height: 40px !important; /* Запрет на растяжение */

  flex-shrink: 0; /* Запрет Flexbox на сжатие */

  /* Сброс отступов и центрирование */
  display: flex;
  align-items: center;
  padding: 0 !important; /* Принудительный сброс внутренних отступов */
  margin: 0 !important; /* Принудительный сброс внешних отступов */
  box-sizing: border-box; /* Гарантирует, что border/padding не увеличивают размер */
}
/* --- 3. ТЕКСТ МЕТОК --- */
.time-header-item {
  position: absolute;
