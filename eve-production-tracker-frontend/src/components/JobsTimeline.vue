...
.timeline-container {
  /* ОЧЕНЬ ВАЖНО: Должен занимать всю доступную высоту */
  height: 100%;
  min-height: 0; /* Предотвращает проблемы в некоторых браузерах */

  display: flex;
  flex-direction: column; /* Вертикальная компоновка: Управление, Скролл */
  overflow: hidden;
  max-width: 100%;
  width: 100%;
}
/* 2. КОНТРОЛЫ (Фиксированная высота, не скроллятся) */
.timeline-controls {
...
/* 3. КОНТЕЙНЕР ВЕРТИКАЛЬНОЙ ПРОКРУТКИ */
.timeline-scroll-wrapper {
  flex: 1; /* 🔑 КРИТИЧНО: Занимает все оставшееся место (100% - controls) */
  overflow: auto; /* Разрешает вертикальную прокрутку */
  position: relative;
  /* Убедитесь, что нет padding/margin, которые могли бы растянуть его */
  padding: 0;
  margin: 0;
}
...
/* 4. ОБЁРТКА ТАЙМЛАЙНА (Внутри скролл-контейнера) */
.timeline-wrapper {
  position: relative;
  min-height: 100%;
  /* 🔑 КРИТИЧНО: Чтобы time-headers и character-rows-container работали */
  display: flex;
  flex-direction: column;
}
/* 5. ШКАЛА ВРЕМЕНИ (HEADER) */
.time-headers {
  position: sticky;
  top: 0;
  ...
  /* Сброс отступов и центрирование текста */
  display: flex;
  align-items: center;
  padding: 0;
  margin: 0;
...
.time-header-item {
  position: absolute;
  top: 0;
  height: 100%; /* Занимает 100% от 40px */
  padding: 0 !important;
  margin: 0 !important;
  display: flex;
  align-items: center;
...
.time-header-item span {
  font-size: 11px;
  line-height: 1 !important; /* Принудительная высота строки */
  padding: 0 !important;
  margin: 0 !important; /* Убираем все отступы текста */
  white-space: nowrap;
  color: #888;
}
.current-time-line {
