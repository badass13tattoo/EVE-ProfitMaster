...
.time-headers {
  ...
  flex-shrink: 0; /* Запрещаем сжатие/растяжение */
  z-index: 2;
  background-color: #1a1a1a;
  border-bottom: 1px solid #3c414d;

  /* 🔑 НОВОЕ КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Используем Flexbox для контроля высоты */
  display: flex;
  align-items: center; /* Центрирует все внутренние элементы вертикально */
  padding: 0; /* Убираем любой внутренний отступ, который может растянуть блок */
}
.time-header-item {
  position: absolute;
  top: 0;
  height: 100%; /* Занимает 100% высоты родителя (40px) */

  /* 🔑 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Сброс внутренних отступов и выравнивание */
  padding: 0 4px;
  margin: 0;
  display: flex; /* Используем Flexbox для центрирования текста внутри метки */
  align-items: center;
...
.time-header-item span {
  /* 🔑 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Минимальный размер и высота строки для текста */
  font-size: 11px;
  line-height: 1;
  color: #888;
  transform: translateY(
    1px
  ); /* Небольшая корректировка для визуального центрирования, если нужно */
}
.current-time-line {
  position: absolute;
  top: 0;
