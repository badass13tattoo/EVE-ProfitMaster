...
.time-headers {
  ...
  min-height: 40px; /* 🔑 НОВОЕ: Запрещаем сжиматься */
  max-height: 40px; /* 🔑 НОВОЕ: Запрещаем растягиваться */
  flex-shrink: 0;
  z-index: 2;
  ...
  display: flex;
  align-items: center;
  padding: 0; /* 🔑 Сброс всех внутренних отступов */
  margin: 0; /* 🔑 Сброс всех внешних отступов */
  box-sizing: border-box;
}
.time-header-item {
  position: absolute;
  top: 0;
  height: 100%; /* 100% от 40px */

  /* 🔑 КРИТИЧЕСКИЙ СБРОС */
  padding: 0; /* Убираем отступы вокруг метки */
  margin: 0;
...
.time-header-item span {
  /* 🔑 КРИТИЧЕСКИЙ СБРОС ДЛЯ ТЕКСТА */
  font-size: 11px;
  line-height: 1; /* Жестко фиксируем высоту строки */
  padding: 0;
  margin: 0; /* Убираем возможные margin от браузера */
  white-space: nowrap;
  color: #888;
}
.current-time-line {
