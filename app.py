...
# --- ИЗМЕНЕНИЕ 1: Отложенная инициализация ---
# Создаем объекты расширений БЕЗ привязки к конкретному приложению
db = SQLAlchemy()
cors = CORS()

# --- ИЗМЕНЕНИЕ 2: Модель данных теперь не зависит от 'app' ---
class User(db.Model):
...
# --- ИЗМЕНЕНИЕ 3: Создание "Фабрики приложений" ---
def create_app():
    ...
    # Получаем и МОДИФИЦИРУЕМ URL внутри фабрики
    ...
    # --- Инициализация расширений с готовым приложением ---
    db.init_app(app)
    cors.init_app(app)

    # --- Регистрация роутов ---
    # Все роуты теперь находятся внутри фабрики, чтобы иметь доступ к 'app'
...
# --- ИЗМЕНЕНИЕ 4: Создаем экземпляр приложения для Gunicorn ---
app = create_app()
